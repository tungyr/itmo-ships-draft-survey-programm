...
class MainWindowBealunaRus(QWidget):
    def __init__(self, parent=None):
        super(MainWindowBealunaRus, self).__init__(parent)
        ...
        for draft_line in self.draft_lines:
            draft_line.setToolTip('Допустимые значения осадок только: 2 - 7.8')
    ...
    def validate_forms(self, draft_lines, stores_lines, density_line) -> (list, list, float):
        ...
        try:
            ...
            warn = QMessageBox.warning(self, 'Message',
                                       "Допустимые значения осадок только: 2 - 7.8"
                                       + "\n" + "Допустимые значения плотности только: 0.1 - 2"
                                       + "\n", QMessageBox.Ok)
            return
